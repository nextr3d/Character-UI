...
class OPS_OT_AddNewAttributeGroup(Operator):
    ...
    def execute(self, context):
        ...
        if ch:
            ...
            def add_group(att):
                att.append({"name": 'Group_'+str(len(att)),
            ...
            if self.panel_name in ch[attributes_key] and len(ch[attributes_key][self.panel_name]):
                ch[attributes_key][self.panel_name] = add_group(
                    ch[attributes_key][self.panel_name])
            else:
                ch[attributes_key][self.panel_name] = add_group([])
...
class OPS_OT_EditAttributeGroup(Operator):
    ...
    group_icon: StringProperty(
        name="Icon", description="Name of the Icon for the group. Enable the built-in addon Icon Viewer to see all of the available icons.")
            ...
            if self.panel_name in ch[attributes_key]:
                ...
                for i in range(len(ch[attributes_key][self.panel_name])):
                    ...
                    if ch[attributes_key][self.panel_name][i]["name"] == self.new_group_name.replace(" ", "_"):
                        self.report(
                            {'INFO'}, "No changes saved, duplicated name for one panel")
                        return {'CANCELLED'}
                    if ch[attributes_key][self.panel_name][i]["name"] == self.group_name:
                        index = i

                ch[attributes_key][self.panel_name][index]["name"] = self.new_group_name.replace(
                    " ", "_")
        ...
        if ch:
            ...
            if self.panel_name in ch[attributes_key]:
                ...
                if self.direction and i-1 >= 0:  # move attribute group up in the list
                    ...
                    att[i] = prev
                    self.report(
                        {'INFO'}, "Moved attribute group up in the list")
                elif not self.direction and i+1 < len(att):
                    next = att[i+1]
                    att[i+1] = att[i]
                    att[i] = next
                    self.report(
                        {'INFO'}, "Moved attribute group down in the list")
